# New conditional training code snippet
...
def sample_masks(x, adj, keep_ratio=0.5):
    """
    Build node_mask and mask_adj for a batch.

    mask convention:
      node_mask == 0 -> observed node
      node_mask == 1 -> unknown node (to inpaint)

      mask_adj == 0 -> observed edge
      mask_adj == 1 -> unknown edge

    Strategy:
      - for each graph, pick a random subset of *real* nodes to KEEP (observed),
      - everything else is unknown,
      - edges between kept nodes are observed, all other edges unknown.
    """
    B, N, F = x.shape
    device = x.device

    node_mask = torch.ones(B, N, device=device)  # 1 = unknown, 0 = observed
    mask_adj = torch.ones(B, N, N, device=device)  # 1 = unknown, 0 = observed

    for b in range(B):
        # real nodes: feature vector not all zeros
        real_nodes = x[b].abs().sum(dim=-1) > 0
        idx = torch.nonzero(real_nodes, as_tuple=False).flatten()
        num_real = idx.numel()
        if num_real == 0:
            continue

        k_keep = max(1, int(np.ceil(keep_ratio * num_real)))
        perm = idx[torch.randperm(num_real, device=device)[:k_keep]]
        kept = perm

        # observed nodes
        node_mask[b, kept] = 0.0

        # observed edges between kept nodes
        mask_adj[b][kept.unsqueeze(0), kept.unsqueeze(1)] = 0.0

    return node_mask, mask_adj
