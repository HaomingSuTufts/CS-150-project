...
def count_across_community_edges(adjs, Zs):
    ...
    community_diff = communities.unsqueeze(-1) != communities.unsqueeze(
        -2
    )  # Shape: (b, n, n)
    ...
    across_community_edges = (
        adjs * community_diff
    )  # Mask adjacency matrix with across-community edges
    across_community_edge_counts = (
        across_community_edges.sum(dim=(1, 2)) / 2
    )  # Sum over n x n for each graph
...
def est_p_intra_inter(graph, communities=None):
    ...
    for com1, com2 in itertools.combinations(communities, 2):
        edges_between += sum(
            1 for u, v in itertools.product(com1, com2) if graph.has_edge(u, v)
        )
        possible_edges_between += len(com1) * len(com2)
    p_between = (
        edges_between / possible_edges_between if possible_edges_between > 0 else 0
    )
...
def is_sbm_graph(
    G, p_intra=None, p_inter=None, strict=True, communities=None, factor=10.0
):
    """
...
def pow_tensor(x, cnum):
    """
    Create higher order adjacency matrices.
    Accepts x either as B x N x N or B x C x N x N.
    If x is 3D (B x N x N), returns B x cnum x N x N where each channel is x^1, x^2, ..., x^cnum.
    If x is 4D (B x C x N x N), returns B x (C * cnum) x N x N where for each channel c
    we expand its powers to length cnum and concatenate along the channel dimension.
    """
    if x.dim() == 3:
        # Single channel: B x N x N -> B x cnum x N x N
        x_ = x.clone()
        xc = [x.unsqueeze(1)]
        ...
        # Multi-channel: B x C x N x N -> B x (C * cnum) x N x N
        B, C, N, _ = x.shape
        xc_list = []
        for c in range(C):
            x_chan = x[:, c, :, :].clone()
            x_pow = [x_chan.unsqueeze(1)]  # B x 1 x N x N
            x_ = x_chan
            for _ in range(cnum - 1):
                x_ = torch.bmm(x_, x_chan)
                x_pow.append(x_.unsqueeze(1))
            xc_list.append(torch.cat(x_pow, dim=1))
        xc = torch.cat(xc_list, dim=1)
        return xc
    else:
        raise ValueError(f"Unsupported tensor dims for pow_tensor: {x.dim()}")
