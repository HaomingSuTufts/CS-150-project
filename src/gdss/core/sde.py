...
class VPSDE(SDE):
  ...
  def obj0estimation(self, obj, score, t):
    sqrt_alpha_bar_t = self.sqrt_alphas_cumprod.to(obj.device)[t]
    alpha_bar_t = self.alphas_cumprod.to(obj.device)[t]
    return 1/sqrt_alpha_bar_t[:, None, None] * (obj + (1 - alpha_bar_t[:, None, None])*score)
...
class VESDE(SDE):
  ...
  def discretize(self, x, t):
    ...
    timestep = (t * (self.N - 1) / self.T).long()
    sigma = self.discrete_sigmas.to(t.device)
    adjacent_sigma = torch.where(timestep == 0, torch.zeros_like(t),
                                 sigma[timestep - 1])
    f = torch.zeros_like(x)
    G = torch.sqrt(sigma[timestep] ** 2 - adjacent_sigma ** 2)
    return f, G
  ...
  def obj0estimation(self, obj, score, t):
    sigma_t = self.discrete_sigmas.to(t.device)[t]# self.sigma_min * (self.sigma_max / self.sigma_min) ** t
    #print("SIGMAS", self.sigma_min, self.sigma_max, t, sigma_t)
    return obj + (sigma_t[:,None,None]**2)*score
