...
class EulerMaruyamaPredictor(Predictor):
    ...
    def guidance(self, x, adj, flags, t, is_adj):
        ...
        if self.guidance_args.method == "greedy":
            ...
            for i in range(n_traj):
                ...
                score_i = (
                    self.score_fn(x, obj_hat, flags, t + dt)
                    if is_adj
                    else self.score_fn(obj_hat, adj, flags, t + dt)
                )

                obj0hat = self.sde.obj0estimation(obj_hat, score_i, timestep)
                obj0hat_masked = (
                    mask_adjs(obj0hat, flags) if is_adj else mask_x(obj0hat, flags)
                )

                losses[i, :] = (
                    loss_fn(x, obj0hat_masked, **loss_kwargs)
                    if is_adj
                    else loss_fn(obj0hat_masked, adj, **loss_kwargs)
                )

            losses_expanded = (
                torch.argmin(losses, dim=0)
                .view(1, obj.shape[0], 1, 1)
                .expand(1, obj.shape[0], obj.shape[1], obj.shape[2])
                .to(obj.device)
            )

            return torch.gather(
                torch.stack(obj_hats, dim=0), 0, losses_expanded
            ).squeeze(0), obj_mean
            ...
            score_no_noise = (
                self.score_fn(x, obj.clone(), flags, t + dt)
                if is_adj
                else self.score_fn(obj.clone(), adj, flags, t)
            )
            obj0hat_no_noise = self.sde.obj0estimation(
                obj.clone(), score_no_noise, timestep
            )
            obj0hat_masked_no_noise = (
                mask_adjs(obj0hat_no_noise, flags)
                if is_adj
                else mask_x(obj0hat_no_noise, flags)
            )
            no_noise_loss = (
                loss_fn(x, obj0hat_masked_no_noise, **loss_kwargs)
                if is_adj
                else loss_fn(obj0hat_masked_no_noise, adj, **loss_kwargs)
            )
            ...
            weights = (
                losses - no_noise_loss[None, :]
            ) / self.guidance_args.delta  # (diffusion[None, :] * np.sqrt(-dt))
            ...
            if self.guidance_args.clip_method == "clip":
                # Gradient clipping
                weighted_directions = torch.clamp(
                    weighted_directions,
                    -self.guidance_args.clip,
                    self.guidance_args.clip,
                )

                obj = obj_mean - self.guidance_args.lr_zero * weighted_directions
            elif self.guidance_args.clip_method == "norm":
                norm_grad_step = torch.linalg.norm(weighted_directions, dim=(1, 2)) / (
                    weighted_directions.shape[1] * weighted_directions.shape[2]
                )
                norm_grad_step = torch.where(
                    norm_grad_step < 1e-7,
                    torch.ones_like(norm_grad_step),
                    norm_grad_step,
                )

                obj = (
                    obj_mean
                    - self.guidance_args.lr_zero
                    * weighted_directions
                    / norm_grad_step[:, None, None]
                )
            else:
                obj = obj_mean - self.guidance_args.lr_zero * weighted_directions
            return obj, obj_mean
            ...
            with torch.enable_grad():
                ...
                score = (
                    self.score_fn(x, obj, flags, t)
                    if is_adj
                    else self.score_fn(obj, adj, flags, t)
                )
            ...
            drift = drift - diffusion[:, None, None] ** 2 * score * (
                0.5 if self.rsde.probability_flow else 1.0
            )
            # -------- Set the diffusion function to zero for ODEs. --------
            diffusion = 0.0 if self.rsde.probability_flow else diffusion
            ...
            raise NotImplementedError(
                f"guidance method {self.guidance_args.method} not yet supported."
            )
    ...
    def update_fn(self, x, adj, flags, t):
        ...
        if self.guidance_args is not None and timestep > 0:
...
class ReverseDiffusionPredictor(Predictor):
    ...
    def guidance(self, x, adj, flags, t, is_adj):
        ...
        if self.guidance_args.method == "greedy":
            ...
            score_no_noise = (
                self.score_fn(x, obj_mean.clone(), flags, t)
                if is_adj
                else self.score_fn(obj.clone(), adj, flags, t)
            )
            obj0hat_no_noise = self.sde.obj0estimation(
                obj_mean.clone(), score_no_noise, timestep
            )
            obj0hat_masked_no_noise = (
                mask_adjs(obj0hat_no_noise, flags)
                if is_adj
            ...
            directions = torch.stack(noise_directions, dim=0)
            obj = obj_mean - self.guidance_args.lr_zero * (
                weights[:, :, None, None] * directions
            ).mean(dim=0)
            return obj, obj_mean
...
def get_pc_sampler(
    ...
    constraint_config_prodigy=None,
    method_config_prodigy=None,
    snr=0.1,
    scale_eps=1.0,
    n_steps=1,
    ...
    def pc_sampler(model_x, model_adj, init_flags):
        ...
        predictor_obj_x = predictor_fn(
            "x", sde_x, score_fn_x, probability_flow, guidance_args=guidance_args_x
        )
        ...
        with torch.no_grad():
            ...
            for i in trange(
                ...
                if (
                    constraint_config_prodigy is not None
                    and method_config_prodigy is not None
                ):
                    x, adj = drifted_project(
                        x,
                        adj,
                        i=i,
                        diff_steps=diff_steps,
                        constraint_config=constraint_config_prodigy,
                        method_config=method_config_prodigy,
                    )
    ...
    def s4_solver(model_x, model_adj, init_flags):
        ...
        with torch.no_grad():
            ...
            for i in trange(
                ...
                if guidance_args_x is not None and timestep > 0:
                    x = apply_guidance(
                        x,
                        adj,
                        flags,
                ...
                if guidance_args_adj is not None and timestep > 0:
                    adj = apply_guidance(
                        x,
                        adj,
                        flags,
                ...
                if (
                    ...
                    x_mean, adj_mean = drifted_project(
                        x_mean,
                        adj_mean,
                        i=i,
                        diff_steps=diff_steps,
...
def apply_guidance(
    ...
    if guidance_args is not None and guidance_args.method == "greedy":
        ...
        for i in range(n_traj):
            ...
            score_i = (
                score_fn(x, obj_hat, flags, vec_t + vec_dt)
                if is_adj
                else score_fn(obj_hat, adj, flags, vec_t + vec_dt)
            )
        ...
        losses_expanded = (
            torch.argmin(losses, dim=0)
            .view(1, x.shape[0], 1, 1)
            .expand(1, x.shape[0], x.shape[1], x.shape[2])
            .to(x.device)
        )
        ...
        score_no_noise = (
            score_fn(x, mu.clone(), flags, vec_t)
            if is_adj
            else score_fn(mu.clone(), adj, flags, vec_t)
        )
